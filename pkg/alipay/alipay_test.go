...
func init() {
	...
	if cert {
		// 使用支付宝证书
		fmt.Println("加载证书", client.LoadAppCertPublicKeyFromFile("./cert/appPublicCert.crt"))
		fmt.Println("加载证书", client.LoadAliPayRootCertFromFile("./cert/alipayRootCert.crt"))
		fmt.Println("加载证书", client.LoadAlipayCertPublicKeyFromFile("./cert/alipayPublicCert.crt"))
	} else {
		// 使用支付宝公钥
